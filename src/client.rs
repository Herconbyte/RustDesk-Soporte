...
impl Client {
    ...
    async fn secure_connection(
        ...
        let rs_pk = get_rs_pk(if key.is_empty() {
            crate::herconbyte_config::RS_PUB_KEY
        } else {
            key
        });
    ...
    pub fn initialize(
        ...
        if id.contains("@") {
            ...
            let key = if server == PUBLIC_SERVER {
                crate::herconbyte_config::RS_PUB_KEY.to_owned()
            } else {
...
async fn hc_connection_(
    ...
    let _key = crate::get_key(true).await;
    /* Herconbyte: secure_tcp deshabilitado — el servidor hbbs propio no lo soporta
       (mismo motivo que en las otras rutas, ver commit d5dc9beac). */
